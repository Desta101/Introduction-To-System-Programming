...
int		saveManagerToFile(const AirportManager* pManager, const char* fileName)
{
	...
	while(pNode)
	{
		CHECK_0_MSG_COLSE_FILE(saveAirportToFile(pNode->key, fp), "Error write airport", fp);
		pNode = pNode->next;
	}
	fclose(fp);
	return 1;
...
int		loadManagerFromFile(AirportManager* pManager, const char* fileName)
{
	...
	for (int i = 0; i < pManager->count; i++)
	{
		...
		CHECK_RETURN_0(pPort);
		CHECK_NULL__MSG_COLSE_FILE(pPort, fp, "Error allocating memeory");
		CHECK_0_MSG_COLSE_FILE(loadAirportFromFile(pPort, fp, "Error loading airport from file"),"Error loading airport from file",fp);
		insertPortToListSorted(&pManager->airportList, pPort);
	}
	fclose(fp);
	return 1;
